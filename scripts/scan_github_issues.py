...
def main() -> int:
    repo = os.getenv("DEMAND_REPO", "KingCrimsonD/ainol-octo-cli-exam-issues").strip()
    token = os.getenv("GITHUB_TOKEN") or os.getenv("GH_TOKEN")
    started = now()
    allowed = os.getenv("ALLOW_WRITE_REPO", repo).strip()
    readonly = os.getenv("READONLY_REPO", "Mininglamp-OSS/octo-cli").strip()
    if repo == readonly or (allowed and repo != allowed):
        append_jsonl(CRON_LOG, {"ts": started, "status": "blocked", "repo": repo, "reason": "repo is not the formal demand-pool allowlist"})
        print("Blocked: DEMAND_REPO is not the formal demand-pool allowlist", file=sys.stderr)
        return 2
