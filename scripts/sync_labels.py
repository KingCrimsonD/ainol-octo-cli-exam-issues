...
repo = os.environ.get("GITHUB_REPO") or os.environ.get("DEMAND_REPO")
allowed = os.environ.get("ALLOW_WRITE_REPO", "KingCrimsonD/ainol-octo-cli-exam-issues")
readonly = os.environ.get("READONLY_REPO", "Mininglamp-OSS/octo-cli")
if not repo:
    print("GITHUB_REPO or DEMAND_REPO is required", file=sys.stderr); sys.exit(2)
if repo != allowed or repo == readonly:
    print(f"Refusing to sync labels for non-allowed repo: {repo}", file=sys.stderr); sys.exit(2)
